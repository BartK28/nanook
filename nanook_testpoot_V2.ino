...
float shoulderSpeedMultiplier = 1;
...
void loop() {
  PidSetGain(PID_ID_SHOULDER, 100 * speedMultiplier * shoulderSpeedMultiplier,50,0);
  PidSetGain(PID_ID_ELBOW, 450 * speedMultiplier * elbowSpeedMultiplier,50,0);
  PidSetGain(PID_ID_ANKLE, 150 * ankleSpeedMultiplier,50,0);
  ...
  outputAnkle = -LimitCalc2(outputAnkle, 16384);

  int shoulderMoving = 1;
  int elbowMoving = 1;
  int ankleMoving = 1;

  if (compareValues(shoulder, shoulderTarget)) {
    shoulderMoving = 0;
  }
  if (compareValues(elbow, elbowTarget)) {
    elbowMoving = 0;
  }
  if (compareValues(ankle, ankleTarget)) {
    ankleMoving = 0;
  }

  //Schrijf PWM uitsturing naar outputs
  ledcWrite(PWM_OUT_SHOULDER, 32767 + outputShoulder * shoulderMoving);
  ledcWrite(PWM_OUT_ELBOW   , 32767 + outputElbow * elbowMoving);
  ledcWrite(PWM_OUT_ANKLE   , 32767 + outputAnkle * ankleMoving);
  ...
  if (!running) {
    ...
    // Set points
    shoulderPosition1 = SHOULDER_MAX_IN + 2; //was 95  -> nu 81
    ...
    // Als de schouder
    if (compareValues(shoulder, shoulderPosition2 + 25) && !goingForward) {
      shoulderSpeedMultiplier = 2;
    }

    if (compareValues(shoulder, shoulderPosition2 + 15) && !goingForward) {
      ankleSpeedMultiplier = 0.05;
      ankleTarget = anklePosition2 + 70;
      Serial.println("1");
    }

    if (compareValues(shoulder, shoulderPosition1)) {
      elbowSpeedMultiplier = 5;
      ankleSpeedMultiplier = 1;
      ankleTarget = anklePosition1;
      elbowTarget = elbowPosition2;
      shoulderSpeedMultiplier = 1;
      Serial.println("2");
    }

    if (compareValues(shoulder, shoulderPosition2 + 25) && goingForward) {
      ankleTarget = anklePosition2+10;
      Serial.println("3");
    }

    if (compareValues(shoulder, shoulderPosition2 + 15) && goingForward) {
      elbowSpeedMultiplier = 2;
      elbowTarget = elbowPosition1;
...
float LimitCalc2(float Value, int Limit)
/***************************************/
//Calculate limit
{
  if (Value >= 0 )
